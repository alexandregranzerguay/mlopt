...
#
# This file contains code from a project licensed under the MIT License.
...
class optimalDecisionTreeClassifier:
    ...
    def __init__(self, n_input, n_classes, max_depth=3, min_samples_split=2, alpha=0, warmstart=True, timelimit=600, output=True, **options):
    ...
    def predict(self, x):
        ...
        """
        if not self.trained:
            raise AssertionError("This optimalDecisionTreeClassifier instance is not trained yet.")
        ...
        for t in self.l_index:
            ...
            while ta != 0:
                if left:
                    ...
                    m.addConstrs(
                        gp.quicksum(a[j, ta] * x[i, j] for j in range(self.p)) >= b[ta] - (1 - z[i, t])
                        for i in range(self.n)
                    )
                left = ta % 2 == 0
                ta //= 2
    ...
    def save(self, file_name):
        if self.trained:
            # write the entire class object to a pickle file
    ...
    def load(self, file_name):
        # read the entire class object from a pickle file
